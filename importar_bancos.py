...
# ==========================================
# EXTRACCIÓN DE DATOS DE BANCOS
# ==========================================

print("Iniciando descarga del modulo de Bancos desde Alegra...")

# Cargar base de datos existente si existe para no borrar clientes ni productos
base_de_datos = {}
if os.path.exists("datos_alegra.json"):
    try:
        with open("datos_alegra.json", "r", encoding="utf-8") as f:
            base_de_datos = json.load(f)
        print("-> Se cargo la base de datos existente 'datos_alegra.json'. Se preservaran clientes y productos.")
    except Exception as e:
...
cuentas_bancarias = consultar_alegra("bank-accounts")

# 2. Transacciones / Pagos (movimientos bancarios de entrada y salida)
print("-> Descargando Transacciones y Pagos bancarios (Movimientos)...")
movimientos_bancarios = consultar_alegra("payments")

# 3. Conciliaciones / Consolidaciones bancarias
print("-> Descargando Conciliaciones Bancarias...")
conciliaciones_bancarias = consultar_alegra("conciliations")

# ==========================================
# GUARDAR O ACTUALIZAR BASE DE DATOS
# ==========================================

base_de_datos["cuentas_bancarias"] = cuentas_bancarias
base_de_datos["movimientos_bancarios"] = movimientos_bancarios
base_de_datos["conciliaciones_bancarias"] = conciliaciones_bancarias

with open("datos_alegra.json", "w", encoding="utf-8") as f:
    json.dump(base_de_datos, f, indent=4, ensure_ascii=False)

print(f"\nProceso del modulo de Bancos completado con exito!")
print(f"Total Cuentas Bancarias / Cajas importadas: {len(cuentas_bancarias)}")
print(f"Total Movimientos / Transacciones bancarias importados: {len(movimientos_bancarios)}")
print(f"Total Conciliaciones Bancarias importadas: {len(conciliaciones_bancarias)}")
