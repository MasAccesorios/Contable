...
$apiUrl = "https://script.google.com/macros/s/AKfycbxrhGrVogJxlZGkVMisyMad-4r-X5eMNl8BtcV2ORNLh4R46KSgYK5fSTrMYe2uozaF/exec"
...
# Invoices
Write-Output "Sincronizando Facturas..."
$invRes = Invoke-RestMethod -Uri "https://api.alegra.com/api/v1/invoices?limit=30&order_direction=DESC&date=$targetDate" -Headers $headers
$todayInvoices = $invRes | Where-Object { $_.date -like "$targetDate*" }
...
# Payments
Write-Output "Sincronizando Pagos..."
$payRes = Invoke-RestMethod -Uri "https://api.alegra.com/api/v1/payments?limit=30&order_direction=DESC&date=$targetDate" -Headers $headers
$todayPayments = $payRes | Where-Object { $_.date -like "$targetDate*" }
...
# Estimates
Write-Output "Sincronizando Cotizaciones..."
$estRes = Invoke-RestMethod -Uri "https://api.alegra.com/api/v1/estimates?limit=30&order_direction=DESC&date=$targetDate" -Headers $headers
$todayEstimates = $estRes | Where-Object { $_.date -like "$targetDate*" }
...
Write-Output "Se ha generado el archivo 'test_sync_results.json' con el volcado de los datos."
