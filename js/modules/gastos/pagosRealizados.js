...
export const PagosRealizadosModule = {
    ...
    async init(element) {
        ...
        window.abrirVistaA4Egreso = async (id, mode = 'preview') => {
            ...
            const htmlContent = `
                <div style="padding: 20px; font-family: 'Inter', sans-serif; color: #333;">
                    <div style="display: flex; justify-content: space-between; border-bottom: 2px solid #e74c3c; padding-bottom: 20px; margin-bottom: 30px;">
                        <div>
                        ...
                        <div style="text-align: right;">
                            <h2 style="margin: 0; font-size: 20px; color: #e74c3c;">COMPROBANTE DE EGRESO</h2>
                    ...
                    <div style="display: flex; justify-content: space-between; margin-bottom: 30px; background-color: #f8f9fa; padding: 15px; border-radius: 8px;">
                        ...
                        <div>
                            <p style="margin: 0 0 5px 0; font-size: 12px; color: #6c757d; font-weight: bold;">PAGADO A</p>
                    ...
                    <div class="no-print" style="margin-top: 40px; text-align: center;">
                        <button onclick="window.editarTransaccionGlobalOut(${t.id})" style="background-color: #fff; border: 1px solid #dee2e6; padding: 8px 16px; border-radius: 6px; cursor: pointer; color: #495057;">
            ...
            PrintManager._renderPreviewShell(htmlContent, { mode, title: 'Comprobante de Egreso', fileName: `comprobante_egreso_${t.numero || t.id}.png` });
        };
        
        window.editarTransaccionGlobalOut = (id) => {
            ...
            <div class="py-3 px-4" style="font-family: 'Inter', sans-serif; background-color: #f8f9fa; min-height: 100vh; font-size: 13px;">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        ...
                        <p class="text-muted m-0 mt-1" style="font-size: 13px;">Historial de todas las salidas de dinero (tipo "Salida").</p>
                    </div>
                </div>
                        ...
                        <table class="table align-middle table-hover m-0 text-nowrap">
                            ...
                            <tbody style="font-size: 13px; color: #2c3e50;">
                                ${this.state.pagos.length > 0 ? this.state.pagos.map(pago => {
                                    ...
                                    if (pago.factura_id) {
                                        detallesHtml = `<a href="#/gastos/proveedores/ver/${pago.factura_id}" class="text-decoration-none text-primary" onclick="event.stopPropagation()">Compra #${pago.factura_numero || pago.factura_id}</a>`;
                                    }

                                    return `
                                        <tr style="cursor: pointer;" onclick="if(!event.target.closest('.btn-menu-row') && !event.target.closest('.dropdown-menu') && !event.target.closest('a')) window.abrirVistaA4Egreso(${pago.id})">
            ...
            btn.addEventListener('click', (e) => {
                ...
                if (actionVer) {
                    actionVer.addEventListener('click', (ev) => {
                        ...
                        ev.stopPropagation();
                        window.cleanupFloatingElements();
                        window.abrirVistaA4Egreso(id, 'preview');
                    });
                }
                ...
                if (actionImprimir) {
                    actionImprimir.addEventListener('click', async (ev) => {
                        ...
                        ev.stopPropagation();
                        window.cleanupFloatingElements();
                        window.abrirVistaA4Egreso(id, 'print');
                    });
                }
